...
class RmspropOptimizer(Optimizer):
    ...
    def update_parameters(self, parameters, grads, L):
        for l in range(L):
            ...
            # update parameters
            parameters['W' + str(l+1)] -= self._learning_rate * grads[dW] / (self._s[dW] ** 0.5 + self._epsilon)
            parameters['b' + str(l+1)] -= self._learning_rate * grads[db] / (self._s[db] ** 0.5 + self._epsilon)
        return parameters